...
public class UserDaoImpl implements UserDao {
	...
	public List<SearchResponsePojo> searchMatch(SearchResultPojo searchMatch) {
		...
		if(searchMatch.getGender()!=null && searchMatch.getFrom()!=null && searchMatch.getCaste()!=null &&searchMatch.getName()==null)
		{
			String caste=searchMatch.getCaste().substring(0, 2);
			query1=session.createQuery("from UserDetails where gender=:gender  and caste like :caste and registerId !=:id");
			
			query1.setParameter("gender",searchMatch.getGender());
			query1.setParameter("id", Integer.parseInt(searchMatch.getUserId()));
			query1.setParameter("caste",caste+"%");
		
		}
		else
		{
			if(searchMatch.getName()!=null)
			{
				query1=session.createQuery("from UserDetails where name=:name  and registerId !=:id");	
				query1.setParameter("id", Integer.parseInt(searchMatch.getUserId()));
				query1.setParameter("name",searchMatch.getName());
			}			
		
			 if(searchMatch.getGender()==null)
			{
				 String caste=searchMatch.getCaste().substring(0, 2);
				query1=session.createQuery("from UserDetails where caste like :caste or name=:name and registerId!=:id");
				query1.setParameter("id", searchMatch.getUserId());
				query1.setParameter("caste",caste);
				query1.setParameter("name",searchMatch.getName());
			}
			...
			{			
				query1=session.createQuery("from UserDetails where gender=:gender or name=:name and  registerId!=:id");
				query1.setParameter("gender",searchMatch.getGender());
				query1.setParameter("id", searchMatch.getUserId());
				query1.setParameter("name",searchMatch.getName());
			}
